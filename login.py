...
conn=sqlite3.connect(r"C:\Users\Klaus\Desktop\store system\Database\store.db")
c=conn.cursor()
...
def home():
    ...
    def Suggestcomposition():
        ...
        if 0 < inserted <= 50:
            ...
            "yellowish leaves - 1st spray(urea, calcium)\n"
            "Second Spray -  use appropriate insectside (Example: if caterpillars exist, use EMAMECTIN BENZOATE and if Leaf Miners appear, use Abamectin\n"
            "Optimum Soil PH: 4.5 to 7.0\n"
            ...
            tBox.delete(1.0, END)
            tBox.insert(END, "ON" + " " + str(season_var.get()) + " " + str(region_var.get()) + " " + "REGION, YEAR" + " " + str(year_entry.get()) + " " + "The Farm has a fertilty level of:"  + str(inserted) + "\n"
            "Favourable for Seed varieties such as (Basmati 370, Basmat 217,\n BW 196, BG-90-2, BR 51-74-6 and IR 2793-80-1)\n"
            ...
            "Chemical application - Use recommended insectside ((Example: if caterpillars exist, use EMAMECTIN BENZOATE and if Leaf Miners appear, use Abamectin)\n" 
            "pH below optium - apply Hydrated lime \n" 
            "pH above optium - apply- aluminum sulfate, iron sulfate \n")
...
def first_win():
    ...
    def generate_bill():
        ...
        directory = "C:/Users\Klaus/Desktop/store system/invoice/" + str(date) + "/"
    ...
    regionLabel_p= Label(left_side,text='Product reference', font=('cambria 14 bold'),bg='yellow',fg='#043927')
    regionLabel_p.place(x=10,y=210)
    ...
    dropDownMenu_p.configure(width=20, font=('arial 10'),highlightbackground="yellow", highlightcolor="red", highlightthickness=2, bd= 0 )
    dropDownMenu_p.place(x=240,y=210)

    search_button=Button(left_side,text="Search",width=15,height=2,bg='green',fg='white',command=ajax)
    search_button.place(x=450,y=210)
